...
export default function AdminProjectsPage() {
  ...
  const filteredProjects = useMemo(() => {
    ...
    return projects;
  }, [projectsData, isAdmin, isAdminOrManager, managedProjectIds, activeFilter, searchQuery]);
  ...
  const columns: Column<HarvestProject>[] = [
    ...
    },
    {
      header: 'Budget',
      accessor: p => {
        const isCostBased = p.budget_by === 'project_cost' || p.budget_by === 'task_fees';
        const isHoursBased =
          p.budget_by === 'project' || p.budget_by === 'task' || p.budget_by === 'person';
        ...
        <div className="text-sm">
          {p.starts_on && p.ends_on ? (
            <>
              ...
              <div className="text-muted-foreground">{format(new Date(p.ends_on), 'PP')}</div>
            </>
          ) : (
      ...
      accessor: p => (
        <div className="flex gap-2 justify-end">
          <Button
            variant="ghost"
            size="icon"
            onClick={() => router.push(`/dashboard/admin/projects/${p.id}/assignments`)}
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Modals */}
      <ProjectFormModal open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen} />
      <ProjectFormModal
        open={!!editProjectId}
