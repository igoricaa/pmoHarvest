 ...
 */
export function groupTimeEntriesByUserAndWeek(
	entries: HarvestTimeEntry[],
): WeeklyTimesheet[] {
 ...
 */
export function groupExpensesByUserAndWeek(
	expenses: HarvestExpense[],
): WeeklyExpenseSheet[] {
 ...
 */
export function createTimesheetGrid(
	entries: HarvestTimeEntry[],
	weekStart: string,
): TimesheetGrid {
	...
	const dailyTotals: TaskDayHours = {
		mon: 0,
		tue: 0,
		wed: 0,
		thu: 0,
	...
	projectMap.forEach((taskMap, projectId) => {
		...
		taskMap.forEach((taskEntries, taskId) => {
			const hoursPerDay: TaskDayHours = {
				mon: 0,
				tue: 0,
				wed: 0,
				thu: 0,
			...
			taskEntries.forEach((entry) => {
				...
				const dayOfWeek = format(
					date,
					"EEE",
				).toLowerCase() as keyof TaskDayHours;
				hoursPerDay[dayOfWeek] += entry.hours;
				dailyTotals[dayOfWeek] += entry.hours;
			});

			const taskTotalHours = Object.values(hoursPerDay).reduce(
				(sum, h) => sum + h,
				0,
			);
