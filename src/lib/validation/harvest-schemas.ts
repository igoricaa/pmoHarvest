...
export const timeEntryCreateSchema = z.object({
  ...
  spent_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Invalid date format (expected YYYY-MM-DD)'),
  hours: z
    .number()
...
export const timeEntryUpdateSchema = z.object({
  ...
  spent_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional(),
...
export const expenseCreateSchema = z.object({
  project_id: z.number().int().positive('Project ID must be a positive integer'),
  expense_category_id: z.number().int().positive('Category ID must be a positive integer'),
  spent_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Invalid date format (expected YYYY-MM-DD)'),
...
export const projectCreateSchema = z.object({
  ...
  name: z.string().min(1, 'Project name is required').max(255, 'Name cannot exceed 255 characters'),
  code: z
    .string()
...
export const userUpdateSchema = z.object({
  ...
  is_active: z.boolean().optional(),
  weekly_capacity: z.number().int().min(0).max(168).optional(), // Max hours in a week
...
export const clientCreateSchema = z.object({
  name: z.string().min(1, 'Client name is required').max(255, 'Name cannot exceed 255 characters'),
  is_active: z.boolean().default(true),
