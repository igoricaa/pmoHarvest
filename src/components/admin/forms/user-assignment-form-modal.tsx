...
export function UserAssignmentFormModal({
  ...
  useEffect(() => {
    if (isEditMode && assignmentsData && open) {
      const assignment = assignmentsData.user_assignments.find(a => a.id === assignmentId);
      if (assignment) {
  ...
  const onSubmit = async (data: UserAssignmentFormData) => {
    try {
      ...
      if (data.budget) {
        payload.budget = Number.parseFloat(data.budget);
      }
      ...
      toast.error(isEditMode ? 'Failed to update assignment' : 'Failed to assign user');
    }
  };
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex items-center justify-between rounded-lg border p-3">
                    <div>
                      <FormLabel className="text-base">Project Manager</FormLabel>
                      <FormDescription className="text-sm">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex items-center justify-between rounded-lg border p-3">
                    <div>
                      <FormLabel className="text-base">Use Default Rates</FormLabel>
                      <FormDescription className="text-sm">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription>Override the user's default rate</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>Budget allocated for this user on this project</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
